  ...
  <div class="graph-panel">
    <div class="panel-header">
      ...
      <div class="header-tools">
        <button class="tool-btn" @click="$emit('refresh')" :disabled="loading" title="Refresh Graph View">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some content still processing, recommend manually refreshing Graph View later</span>
...
const wasSimulating = ref(false) // Track if was simulating
...
const renderGraph = () => {
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Dynamically adjust distance based on edge count
      // Base distance 150, add 40 per edge
  ...
  const getLinkPath = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop: draw arc from nodes and back
      const loopRadius = 30
      // Start from right of nodes, circle back
  ...
  const getLinkMidpoint = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop label position: right of nodes
      return { x: sx + 70, y: sy }
    }
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .on('click', (event, d) => {
      ...
      // Highlight edges connected to this nodes
