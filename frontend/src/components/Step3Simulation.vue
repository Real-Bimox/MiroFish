...
// Track the last round per platform to detect changes and log them
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect if simulation is complete (via runner_status or platform complete status)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Extra check: if the backend hasn't updated runner_status yet but platforms already report complete
...
const checkPlatformsCompleted = (data) => {
  ...
  // Determine if platform is enabled via actions_count (if count > 0 or running was ever true)
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Do not auto-scroll, let the user freely view the timeline
