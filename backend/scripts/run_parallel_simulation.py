...
# Add filter immediately when module loads, ensuring it takes effect before camel code executes
logging.getLogger().addFilter(MaxTokensWarningFilter())


def disable_oasis_logging():
    """
    Disable OASIS library's detailed log output
...
class ParallelIPCHandler:
    ...
    async def process_commands(self) -> bool:
        ...
        if command_type == CommandType.INTERVIEW:
            ...
            self.send_response(command_id, "completed", result={"message": "Environment will be closed"})
            return False
...
def fetch_new_actions_from_db(
    ...
    """
    Get new action records from database, and supplement with complete context information
...
def _enrich_action_context(
    ...
    try:
        ...
        # Repost: supplement original post content and author
            ...
            if new_post_id:
                # The original_post_id of repost points to original post
                cursor.execute("""
        ...
        # Context supplement failure doesn't affect main flow
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    If boost LLM is configured, different platforms can use different API providers during parallel simulation, improving concurrency.
...
def setup_signal_handlers(loop=None):
    ...
    2. Allow program to clean up resources properly (close database, environment, etc.)
    3. Then exit
    """
