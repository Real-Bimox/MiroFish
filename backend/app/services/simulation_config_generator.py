...
Implements full automation without manual parameter configuration

Adopts step-by-step generation strategy to avoid failures from generating overly long content at once:
...
from .graphiti_entity_reader import EntityNode, ZepEntityReader
...
CHINA_TIMEZONE_CONFIG = {
    ...
    "activity_multipliers": {
        "dead": 0.05,      # Almost no one at early morning
        "morning": 0.4,    # Gradually becoming active in morning
...
class TimeSimulationConfig:
    """Time simulation configuration (based on Chinese daily routine habits)"""
...
class PlatformConfig:
    ...
    # Viral threshold (number of interactions to trigger spread)
...
class SimulationConfigGenerator:
    ...
    def generate_config(
        ...
        # ========== Step 3-N: Batch Generate Agent Configurations ==========
        all_agent_configs = []
        ...
        logger.info(f"Simulation config generation completed: {len(params.agent_configs)} Agent configurations")
...
- User group is Chinese, needs to conform to Beijing time daily routine
- Almost no activity at 0-5 AM (activity multiplier 0.05)
- Gradually becoming active at 6-8 AM (activity multiplier 0.4)
- Moderately active during work hours 9-18 (activity multiplier 0.7)
- Peak hours at 19-22 (activity multiplier 1.5)
...
- **Important**: The following example values are for reference only, you need to adjust specific time periods based on event nature and participant characteristics
  - For example: student group peak may be 21-23; media active all day; official institutions only during work hours
  - For example: sudden hot topics may lead to late night discussions, off_peak_hours can be shortened appropriately
...
- reasoning (string): Brief explanation of why this configuration"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format, time configuration should conform to Chinese daily routine habits."
...
- **Time should conform to Chinese daily routine**: Almost inactive at 0-5 AM, most active at 19-22 PM
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<active hour list, consider Chinese daily routine>],
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON, configuration should conform to Chinese daily routine habits."
