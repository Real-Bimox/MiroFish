...
2. Plan directory structure first, then generate content section by section
3. Each section uses ReACT multi-round thinking and reflection mode
4. Support conversation with users, autonomously calling retrieval tools during dialogue
"""
...
class ReportLogger:
    ...
    def log_section_content(
        ...
        """Record section content generation completion (content only, not the whole section)"""
        self.log(
...
Lightweight fast search tool, suitable for simple and direct information queries.
...
This is not LLM simulation, but calling real interview interfaces to get raw responses from simulation Agents.
...
4. Call /api/simulation/interview/batch API to conduct real interviews on both platforms
5. Integrate all interview results, providing multi-perspective analysis

[Usage Scenarios]
- Need to understand event perspectives from different role viewpoints (What do students think? What does media say? What do officials say?)
...
- Want to make reports more vivid with "interview records"

[Returns]
- Interviewed Agent identity information
- Interview responses from Agents on both Twitter and Reddit platforms
...
PLAN_SYSTEM_PROMPT = """\
You are an expert in writing "Future Prediction Reports", possessing a "God's-eye view" of the simulation world - you can insight into every Agent's behavior, speech, and interaction in the simulation.

[Core Concept]
We have built a simulation world and injected specific "simulation requirements" as variables into it. The evolution result of the simulation world is a prediction of what may happen in the future. You are observing not "experimental data", but a "rehearsal of the future".
...
- ✅ Agent words and deeds in the simulation world are predictions of future population behavior
...
[Reminder again] Report section count: minimum 2, maximum 5, content should be concise and focus on core prediction discoveries."""
...
1. [Must call tools to observe the simulation world]
   ...
   - All content must come from events and Agent words/deeds in the simulation world
...
2. [Must quote Agent's original words and deeds]
   - Agent speeches and behaviors are predictions of future population behavior
   - Use quote format in reports to display these predictions, for example:
     > "A certain population expressed: original content..."
...
3. [Language Consistency - Quote content must be translated to report language]
   ...
   - If simulation requirements and source materials are in Chinese, the report must be entirely written in Chinese
   - When quoting English or mixed Chinese-English content from tool results, you must translate it into fluent Chinese before writing to the report
   - Maintain original meaning during translation, ensure natural and smooth expression
   - This rule applies to both main text and content in quote blocks (> format)
...
- ❌ Prohibited from using any Markdown headings in sections (#, ##, ###, ####, etc.)
- ❌ Prohibited from adding section main title at the beginning of content
...
**First Outbreak Stage**

Weibo served as the first scene of public opinion, bearing the core function of information first release:

> "Weibo contributed 68% of first-release voice..."
...
### 1. First Outbreak Stage   ← Error! Do not use ### for subsections
#### 1.1 Detailed Analysis    ← Error! Do not use #### for detailed division
...
- insight_forge: Deep insight analysis, automatically decompose questions and retrieve facts and relationships from multiple dimensions
...
Option A - Call tool:
Output your thinking, then use the following format to call a tool:
<tool_call>
{{"name": "tool_name", "parameters": {{"parameter_name": "parameter_value"}}}}
</tool_call>
The system will execute the tool and return results to you. You do not need and cannot write tool return results yourself.
...
- Prohibited from including both tool calls and Final Answer in one reply
...
3. Use Markdown format (but prohibited from using headings):
   ...
   - ❌ Prohibited from using any heading syntax like #, ##, ###, ####
4. [Quote Format Specification - Must be separate paragraphs]
   Quotes must be independent paragraphs, with one empty line before and after, cannot be mixed in paragraphs:
...
- ✅ Write body content directly, use **bold** instead of subheadings
...
REACT_UNUSED_TOOLS_HINT = "\n💡 You haven't used yet: {unused_list}, recommend trying different tools to get multi-perspective information"
...
[Answer Style]
- Concise and direct, don't be lengthy
...
class ReportAgent:
    ...
    # Valid tool names set, used for validating bare JSON fallback parsing
        ...
        for iteration in range(max_iterations):
            ...
            if has_tool_calls and has_final_answer:
                ...
                logger.warning(
                    ...
                    f"LLM outputs both tool calls and Final Answer (conflict #{conflict_retries})"
                )
            ...
            if has_final_answer:
                # Tool calls insufficient, reject and require continuing to call tools
            ...
            # Directly use this content as final answer, no more empty iterations
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        Args:
            report_id: Report ID
            from_line: Which line to start reading from (for incremental fetching, 0 means from beginning)
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        1. Remove Markdown heading lines at content beginning that duplicate section title
    ...
    def assemble_full_report(cls, report_id: str, outline: ReportOutline) -> str:
        ...
        # Post-process: clean up heading issues in entire report
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        3. Clean up excessive empty lines and separators
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check if it's a duplicate heading (same content heading appears within consecutive 5 lines)
                is_duplicate = False
                ...
                if level == 1:
                    if title == outline.title:
                        ...
                        # Section heading incorrectly used #, fix to ##
    ...
    def get_report(cls, report_id: str) -> Optional[Report]:
        ...
        if not os.path.exists(path):
            # Compatible with old format: check files directly stored in reports directory
