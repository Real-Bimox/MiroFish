...
# Adding this prefix prevents the Agent from calling tools and directly replies with text
    ...
    Request (JSON):
        {
            ...
            "graph_id": "mirofish_xxxx",    // Optional, will be fetched from project if not provided
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note: Runtime scripts (run_*.py) are kept in backend/scripts/ directory, not copied to simulation directory
    ...
    try:
        ...
        try:
            ...
            filtered_preview = reader.filter_defined_entities(
                ...
                enrich_with_edges=False  # Do not fetch edge info, speed up
            )
            ...
            if simulation_id:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "not_started",
                        "progress": 0,
                        "message": "Preparation not started yet, call /api/simulation/prepare to start",
                        "already_prepared": False
                    }
                })
            return jsonify({
...
def get_simulation_history():
    ...
    For homepage historical project display, returns simulation list with rich information including project name, description, etc.
    ...
    try:
        ...
        for sim in simulations:
            ...
            if run_state:
                ...
                # Use user-set total_rounds, if none then use recommended rounds
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    - Directly reads files, not through SimulationManager
...
def get_simulation_config(simulation_id: str):
    ...
    Returns include:
        ...
        - agent_configs: Each Agent's activity config (activity level, speech frequency, stance, etc.)
...
def download_simulation_script(script_name: str):
    """
    Download simulation runtime script files (generic scripts located in backend/scripts/)
...
def start_simulation():
    ...
    About enable_graph_memory_update:
        - When enabled, all Agent activities in simulation (posts, comments, likes, etc.) will be updated to Zep graph in real-time
    ...
    try:
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "Enabling graph memory update requires a valid graph_id, please ensure project graph is built"
                }), 400
...
def get_run_status_detail(simulation_id: str):
    """
    Get simulation run detailed status (includes all actions)
    ...
    try:
        ...
        # Get actions for current round (recent_actions only shows latest round)
        ...
        # recent_actions only shows content from both platforms in current latest round
...
def get_agent_stats(simulation_id: str):
    ...
    For frontend Agent activity ranking, action distribution display
    """
    try:
        ...
        if not os.path.exists(db_path):
            return jsonify({
                ...
                "data": {
                    ...
                    "count": 0,
                    "posts": [],
                    "message": "Database does not exist, simulation may not have started yet"
                }
            })
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be running (entering command waiting mode after simulation loop completes)
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // Required, simulation ID
            "prompt": "What is your overall opinion on this matter?",  // Required, interview question (all Agents use same question)
