...
def generate_report():
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "Missing graph_id, please ensure graph is built"
            }), 400
...
def get_generate_status():
    ...
    try:
        ...
        # If simulation_id is provided, check if report is already completed
        if simulation_id:
            existing_report = ReportManager.get_report_by_simulation(simulation_id)
            if existing_report and existing_report.status == ReportStatus.COMPLETED:
                return jsonify({
...
def get_report_sections(report_id: str):
    """
    Get generated sections list (sectioned output)
...
def get_agent_log(report_id: str):
    ...
    Query parameters:
        from_line: Start reading from line (optional, default 0, used for incremental fetch)
...
def get_console_log(report_id: str):
    ...
    which is different from the structured JSON logs returned by the agent-log endpoint.
