...
def generate_ontology():
    """
    Interface 1: Upload file and generate ontology definition
    ...
    try:
        ...
        def build_task():
            ...
            try:
                ...
                # Build graph (progress from 10% to 95%)
                task_manager.update_task(
                    task_id,
                    message=f"Starting graph build, total {total_chunks} text chunks...",
                    progress=10
                )

                def build_progress_callback(processed, total):
                    progress = 10 + int((processed / total) * 85)  # 10% - 95%
                    task_manager.update_task(
                        task_id,
